...
// Generated 11-Nov-2021 21:35:53
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"accel\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(accel_M, (NULL));
        accel_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(accel_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(accel_M)
                == (NULL)) || rtmGetStopRequested(accel_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(accel_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
